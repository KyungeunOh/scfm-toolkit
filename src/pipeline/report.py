...
여기서는 다음을 표준 output으로 정의한다:
  ...
  output_dir/
    ...
    metrics.json             - 전체 accuracy 등 요약 지표
...
def save_metrics(adata_result, output_dir: Path, celltype_col: Optional[str]) -> Optional[Dict]:
    """
    label(celltype_col)이 있으면:
      - metrics.json (accuracy 등 요약)
      - per_class_metrics.csv (precision/recall/f1 per class)
      - confusion_matrix.png
    label이 없으면 조용히 건너뛴다 (label 없는 순수 예측 상황도 있으므로).
    """
    if not celltype_col or celltype_col not in adata_result.obs.columns:
        logger.info("label column이 없어 metrics 계산을 건너뜁니다 (예측 전용 실행).")
        return None
    if "predictions" not in adata_result.obs.columns:
        return None
